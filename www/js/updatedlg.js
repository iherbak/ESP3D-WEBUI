...
function updatedlg() {
    ...
    if (target_firmware ==  firmwares.GrblEmbedded) document.getElementById('fw_update_dlg_title').innerHTML = translate_text_item("ESP3D Update").replace("ESP3D", "GRBL_ESP32");
    if (target_firmware ==  firmwares.MarlinEmbedded) document.getElementById('fw_update_dlg_title').innerHTML = translate_text_item("ESP3D Update").replace("ESP3D",  firmwares.Marlin);
    showModal();
}
