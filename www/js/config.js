...
function refreshconfig(is_override) {
    ...
    if ((target_firmware ==  firmwares.Grbl) || (target_firmware ==  firmwares.GrblEmbedded)) commandtxt = "$$";
    if ((target_firmware ==  firmwares.Marlin) || (target_firmware ==  firmwares.MarlinKimbra) || (target_firmware ==  firmwares.MarlinEmbedded)) commandtxt = "M503";
...
function getprinterconfig(is_override) {
    ...
    if ((target_firmware ==  firmwares.GrblEmbedded) || (target_firmware ==  firmwares.MarlinEmbedded)) SendGetHttp(url);
...
function build_HTML_config_list() {
    ...
    for (var i = 0; i < array_len; i++) {
        ...
        if (item.showcomment) {
            ...
            if ((target_firmware ==  firmwares.Marlin) || (target_firmware ==  firmwares.MarlinKimbra) || (target_firmware ==  firmwares.MarlinEmbedded) || is_override_config) content += "25em";
            else content += "auto";
            ...
            content += "</td>";
            content += "<td style='vertical-align:middle'>";
            if ((target_firmware ==  firmwares.Grbl) || (target_firmware ==  firmwares.GrblEmbedded))content += item.help;
        ...
        if (target_firmware ==  firmwares.Smoothieware) {
            ...
            if (!is_override_config) {
                ...
                if (is_config_override_file()) {
                    ...
                    document.getElementById('config_override_file_name').innerHTML =  firmwares.Smoothieware;
                } else {
...
function config_check_value(value, index, is_override) {
    var isvalid = true;
    if ((target_firmware ==  firmwares.Smoothieware) && !is_override) {
...
function process_config_answer(response_text) {
    ...
    if (tlines.length <= 3) {
        if ((target_firmware ==  firmwares.Smoothieware) && (commandtxt != "cat /sd/config.txt")) {
            if (!is_override_config) {
            ...
            if ((target_firmware ==  firmwares.Smoothieware)) document.getElementById('config_status').innerHTML = translate_text_item("File config / config.txt not found!");
...
function create_config_entry(sentry, vindex) {
    ...
    if ((target_firmware ==  firmwares.Marlin) || (target_firmware ==  firmwares.MarlinKimbra) || (target_firmware ==  firmwares.MarlinEmbedded)) {
...
function get_config_label(sline) {
    ...
    if ((target_firmware ==  firmwares.Smoothieware) || (target_firmware ==  firmwares.Marlin) || (target_firmware ==  firmwares.MarlinKimbra) || (target_firmware ==  firmwares.MarlinEmbedded)) {
        return tline[0];
    }
    if ((target_firmware ==  firmwares.Grbl) || (target_firmware ==  firmwares.GrblEmbedded)) {
...
function get_config_value(sline) {
    var tline = sline.trim().split(" ");
    if ((target_firmware ==  firmwares.Smoothieware) && !is_override_config) {
        if ((tline.length > 1) && tline[0][0] != '#') return tline[1];
        else return "???";
    }
    if ((target_firmware ==  firmwares.Marlin) || (target_firmware ==  firmwares.MarlinKimbra) || (target_firmware ==  firmwares.MarlinEmbedded) || is_override_config) {
...
function is_config_commented(sline) {
    ...
    if ((target_firmware ==  firmwares.Marlin) || (target_firmware ==  firmwares.MarlinKimbra) || (target_firmware ==  firmwares.MarlinEmbedded) || (target_firmware ==  firmwares.Smoothieware)) {
